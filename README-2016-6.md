...
25. 日志模块：提供日志服务，通过dubbo服务(rpc)来打印日志信息(dubbo服务宕机之后，本地是否需要继续记录日志。如果dubbo服务不宕机，是否同时存储小量的日志信息在本机)。具体日志服务目前实现的功能是：1.解决把预处理的sql拼接成完整sql。2.提供日志上下文的依赖关系，方便分析日志堆栈。3.组装更详细的日志信息，避免无法定位到错误代码，或是错误代码块，是否和第二点合并。4.提供其他日志报表。5.日志已经要提供代码行数(类到方法到代码行)