...
import com.meizu.demo.mvc.model.WriteModel;
...
public class WriteAction extends BaseServlet<WriteModel> {
	private static final long serialVersionUID = 4769858916633847282L;
	
	@RequestMap(path = "/(.+)/(.+)/w/(.+)$")
	public IForward doW(HttpServletRequest request, HttpServletResponse response, WriteModel model, @RequestParam(defaultValue = "0", param = "1") String enc, @RequestParam(defaultValue = "0", param = "2") String pid, @RequestParam(defaultValue = "0", param = "3") String id) throws ServletException, IOException, InterruptedException {
...
//		model.setDomain(request.getServerName());
	 ...
	 */
	@Override
	public boolean checkPermission(HttpServletRequest request, HttpServletResponse response, WriteModel model) throws ServletException, IOException {
		// 检查用户登录标记
