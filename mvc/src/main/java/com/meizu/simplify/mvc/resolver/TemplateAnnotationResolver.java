...
public class TemplateAnnotationResolver implements IAnnotationResolver<Class<?>>{
	...
	public static Map<String, ITemplate> templateMap = new ConcurrentHashMap<>();
	...
	public void resolve(List<Class<?>> resolveList) {
		...
		for (Object beanObj : containerCollection) {
			...
			Class<?> itClazz = CollectionUtil.getItem(interfaces, ITemplate.class, (i,t) -> i == t);
			if(itClazz == null) {
				continue;
			}
			
			String templateTypeTag = templateType.value();
			templateMap.put(templateTypeTag , (ITemplate)beanObj);
